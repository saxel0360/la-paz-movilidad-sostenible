...
export const MINIBUS_ROUTES = {
    ...
    'ruta_22': {
        id: 'ruta_22',
        nombre: 'Ruta 22',
        descripcion: 'El Alto (Ceja) → Centro La Paz',
        ...
        frecuencia: '5-8 min',
        paradas: [
            { lat: -16.5180, lng: -68.1650, nombre: 'Ceja El Alto (Terminal)' },
            { lat: -16.5150, lng: -68.1600, nombre: 'Av. 6 de Marzo' },
            { lat: -16.5120, lng: -68.1550, nombre: 'Av. Juan Pablo II' },
            { lat: -16.5080, lng: -68.1500, nombre: 'Av. Naciones Unidas' },
...
export const getMinibusSegment = (routeId, fromStop, toStop, direction = 'forward') => {
    ...
    // 🔧 Función MEJORADA para encontrar índice de parada
    const findStopIndex = (stop) => {
        // Si es string, buscar por nombre exacto o parcial
        if (typeof stop === 'string') {
            ...
            console.warn(`Parada "${stop}" no encontrada en ruta ${routeId}`);
            return -1;
        }
        ...
        // Buscar por coordenadas (si las tiene)
        ...
        console.warn(`Parada no encontrada:`, stop);
        return -1;
    };
    ...
    if (fromIndex === -1 || toIndex === -1) {
        console.warn(`No se encontraron paradas: from=${fromIndex}, to=${toIndex}`);
        console.warn(`From:`, fromStop);
        console.warn(`To:`, toStop);
        console.warn(`Paradas disponibles:`, stops.map(s => s.nombre));
        
        // 🔧 FALLBACK: Si no encuentra las paradas exactas, usar las más cercanas
        // Buscar la parada más cercana por nombre parcial
        const findClosestStop = (searchStop) => {
            const searchName = searchStop.nombre || searchStop;
            // Buscar cualquier parada que contenga parte del nombre
            const words = searchName.split(' ');
            for (const word of words) {
                if (word.length < 3) continue;
                const idx = stops.findIndex(s => 
                    s.nombre.toLowerCase().includes(word.toLowerCase())
                );
                if (idx !== -1) return idx;
            }
            return -1;
        ...
        const fallbackFrom = findClosestStop(fromStop);
        const fallbackTo = findClosestStop(toStop);
        
        if (fallbackFrom !== -1 && fallbackTo !== -1) {
            console.log(`✅ Usando fallback: from=${fallbackFrom}, to=${fallbackTo}`);
            // Usar los índices de fallback
            return getSegmentFromIndices(route, fallbackFrom, fallbackTo, direction);
        }
