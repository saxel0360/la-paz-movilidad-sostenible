...
const {
    ...
    // ============================================
    // CONEXIÓN CON RouteResultsScreen
    // ------------------------------------------------
    // RouteResultsScreen trabaja con datos de demostración
    // (src/data/routes.js) que no tienen coordenadas reales todavía
    // -eso lo resuelve el motor del Desarrollador 1 más adelante-.
    // Mientras tanto, mostramos en el mapa el caso de prueba real
    // más parecido según el medio de transporte elegido, para que
    // el flujo completo (buscar → elegir → ver mapa) no quede cortado.
    // ============================================
    const TEST_CASE_BY_TRANSPORT = {
        MINIBUS: 'estudiante',
        TELEFERICO: 'teleferico_morado',
        PUMAKATARI: 'complejo_teleferico_minibus_puma',
        TRUFI: 'estudiante',
        MICRO: 'trabajador',
    };

    useEffect(() => {
        const selectedRoute = navigationRoute?.params?.selectedRoute;
        if (!selectedRoute) return;

        const primaryTransport = selectedRoute.transportesUsados?.[0];
        const testCaseKey = TEST_CASE_BY_TRANSPORT[primaryTransport] || 'estudiante';
        const testCase = TEST_CASES[testCaseKey];

        if (testCase) {
            handleTestCaseSelect(testCase);
        }
    }, [navigationRoute?.params?.selectedRoute]);
